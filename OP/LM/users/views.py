...
def get_user_money(user):
    money = (Purse.objects.get(user=user).money).encode('ascii')
    print(money)
...
def register(request):
    if request.method == "POST":
        ...
        if not error_message:
            ...
            user.password = (encrypt(user_id, hashlib.sha256(password1.encode('ascii')).hexdigest(), username)).decode('ascii')
            # print(user.id, user.username, user.password, user.first_name, user.last_name, user.email)
            user.save()
            purse = Purse(user=user)
            purse.save()
            purse.money = (encrypt(user_id, purse.money, username)).decode('ascii')
            purse.save()
            print('user created')
...
def crypto(request):
    ...
    # purse = Purse.objects.get(user=user)
    # purse.money = (encrypt(user.id, str(purse.money), user.username)).decode('ascii')
    # purse.save()
...
def pass_crypto(request):
    user = User.objects.get(id=1)
    password = "T@bi2001"
    user_hashed = hashlib.sha256(password.encode('ascii')).hexdigest()
    print("hashed", user_hashed)
    user.password = (encrypt(user.id, user_hashed, user.username)).decode('ascii')
    user.save()
    return redirect('/')
...
#         value = {'username': username}
#         if user is not None:
#             password_dec = decrypt(user.password, user.username)
#             if user.password_ != password_dec:
#                 messages.warning(request, 'Invalid password')
#             auth.login(request, user)
#             return redirect('/')
#         else:
